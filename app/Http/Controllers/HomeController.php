...
class HomeController extends BaseController
{
    ...
    /* ===============================  getResepsionis ================== */
    /* =============================== ================== =============== */
    public function getIframeEvent(Request $request)
    {
      $base = $this->base();
      $baseApp = $this->baseApp();

      $event = expoEvent::where('status', 1)->whereNull('deleted_at')->orderBy('start_event', 'desc')->get();
      return view($this->thema() . 'iframe-event', compact('base', 'baseApp', 'event') );
    }
    ...
    public function getResepsionis(Request $request){
      ...
      foreach ($dev as $key => $val) {
        $dev_mod[ \Illuminate\Support\Str::slug($val->name, '-')] = $val->toArray();
      }
      $dev = $dev_mod;

      return view('pano.resepsionis', compact( 'base', 'baseApp', 'sponsor', 'title', 'txt', 'folder_asset', 'dev') );
    }
